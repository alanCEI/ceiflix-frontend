...
function MovieCard({ movie, onWatchedToggle, isWatched, context }) {
  ...
  return (
    <Link
      to={`/movie/${movie.id}`}
      className="block bg-gray-800 rounded-lg overflow-hidden shadow-lg transition-transform hover:scale-105 group"
    >
      <div className="relative">
        ...
        {context.user && onWatchedToggle && (
          <button
            ...
            className={`absolute top-2 right-2 p-2 rounded-full text-white transition-opacity duration-300 md:opacity-0 group-hover:opacity-100 ${
              isWatched
