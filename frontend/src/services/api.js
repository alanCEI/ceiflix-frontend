...
const fetchFromAPI = async (endpoint, options = {}) => {
  if (!API_TOKEN || API_TOKEN === 'TU_TOKEN_API_AQUÍ') {
    throw new Error('El token de API de TMDB no está configurado. Revisa tu archivo .env.');
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(errorData.status_message || 'Error al conectar con la API de TMDB.');
  }
...
const processMovieData = (movie) => ({
  id: movie.id,
  title: movie.title,
  poster: movie.poster_path ? `${IMAGE_BASE_URL}w500${movie.poster_path}` : 'https://placehold.co/500x750?text=No+Poster',
  backdrop: movie.backdrop_path ? `${IMAGE_BASE_URL}w1280${movie.backdrop_path}` : 'https://placehold.co/1280x720?text=No+Backdrop',
...
export const getPopularMovies = async (options) => {
  const data = await fetchFromAPI('movie/popular?language=es-ES&page=1', options);
